...
$sql = "SELECT * FROM predmeti WHERE id='$predmet_id'";
$result = mysqli_query($conn, $sql);
$predmet = mysqli_fetch_assoc($result);
...
if(isset($_POST['posodobi'])) {
    ...
    $sql = "UPDATE predmeti SET ime='$ime_predmeta', letnik_id='$letnik_id', kljuc='$kljuc' WHERE id='$predmet_id'";
    if(mysqli_query($conn, $sql)) {
        $sporocilo = "Predmet uspešno posodobljen.";
    } else {
        $napaka = "Napaka pri posodobitvi predmeta: " . mysqli_error($conn);
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Uredi Predmet - Noodle</title>
    ...
    <div class="container mx-auto mt-10">
        <div class="w-full max-w-md mx-auto">
            ...
            <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="POST">
                <h2 class="text-center text-2xl mb-6">Uredi Predmet</h2>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Ime Predmeta</label>
                    <input type="text" name="ime_predmeta" value="<?php echo $predmet['ime']; ?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                </div>
                <div class="mb-6">
                    ...
                    <select name="letnik_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                        <?php while($letnik = mysqli_fetch_assoc($result_letniki)): ?>
                            <option value="<?php echo $letnik['letnik_id']; ?>" <?php if($predmet['letnik_id'] == $letnik['letnik_id']) echo 'selected'; ?>>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="kljuc" value="<?php echo $predmet['kljuc']; ?>" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
